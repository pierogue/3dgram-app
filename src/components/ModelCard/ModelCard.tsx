...
import { CubeIcon } from "@/app/_assets/icons/CubeIcon"

export const ModelCard = ({model}: {model: modelDto}) => {
  const { getModelCover } = useServer()
  const [cover, setCover] = useState(null)

  useEffect(()=>{
    getModelCover(model.modelID)
      .then((res)=>{
        fetch(res.data)
        .then((result)=>{
  ...
  return (
    <Link href={`/model-page/${model.modelID}`}>
      <Card className="" type="plain" style={{width: 200, height: 280,
        ...
        {cover ? <img
          ...
          /> : <CubeIcon color="var(--tgui--accent_text_color)"/>}
        <CardCell
          readOnly
          subtitle={`${model.description} `}
          style={{backgroundColor: "var(--tg-theme-bg-color)", height: 70, alignItems: 'flex-start', overflow: 'hidden'}}
          >
          {model.title}
        </CardCell>
      </Card>
